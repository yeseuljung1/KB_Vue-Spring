...
<template>
    ...
    <div class="my-3 d-flex justify-content-between">
        ...
        <div><i class="fa-regular fa-clock"></i> {{ moment(article.updateDate).format('YYYY-MM-DD HH:mm') }}</div>
    </div>
    <hr />

    <!-- 첨부 파일 목록 -->
    <div v-if="article.attaches" class="text-end">
        <div v-for="file in article.attaches" :key="file.no" class="attach">
            <span @click="download(file.no)"></span>
            <span> <i class="fa-solid fa-paperclip"></i> {{ file.filename }} </span>
        </div>
    </div>
